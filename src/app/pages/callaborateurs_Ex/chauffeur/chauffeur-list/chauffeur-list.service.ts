...
import { Table, SearchResult } from "./chauffeur-list.model";
import { tableData } from "./data-chauffeur";
import { SortDirection } from "./chauffeur-list.directive";
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
  return (
    ...
    tables.dateNaissance.toLowerCase().includes(term) ||
    tables.telephone.toString().toLowerCase().includes(term) ||
